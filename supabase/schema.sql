...
create index if not exists quiz_sessions_room_status_idx on public.quiz_sessions(room_id, status, created_at desc);
...
create policy "members can read quiz sessions" on public.quiz_sessions for select to authenticated using (public.is_room_member(room_id));
drop policy if exists "members can write quiz sessions" on public.quiz_sessions;
create policy "members can write quiz sessions" on public.quiz_sessions for all to authenticated using (public.is_room_member(room_id)) with check (public.is_room_member(room_id));

drop policy if exists "members can read quiz answers" on public.quiz_answers;
create policy "members can read quiz answers" on public.quiz_answers for select to authenticated using (public.is_room_member(room_id));
drop policy if exists "players can create own quiz answers" on public.quiz_answers;
create policy "players can create own quiz answers" on public.quiz_answers for insert to authenticated
with check (
